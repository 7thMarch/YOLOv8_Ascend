...
def non_max_suppression(
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        if multi_label:
            i, j = (x[:, 5:mi] > conf_thres).nonzero(as_tuple=False).T # (38) (38)
            x = torch.cat((box[i], x[i, 5 + j, None], j[:, None].float(), mask[i]), 1) # (38,6)
        else:  # best class only
            conf, j = x[:, 5:mi].max(1, keepdim=True) # (38,1) (38,1)
            x = torch.cat((box, conf, j.float(), mask), 1)[conf.view(-1) > conf_thres] # (38,6)
...
# def scale_coords_v8(img1_shape, coords, img0_shape, ratio_pad=None):
#     # Rescale coords (xyxy) from img1_shape to img0_shape
#     if ratio_pad is None:  # calculate from img0_shape
#         gain = min(img1_shape[0] / img0_shape[0], img1_shape[1] / img0_shape[1])  # gain  = old / new
#         pad = (img1_shape[1] - img0_shape[1] * gain) / 2, (img1_shape[0] - img0_shape[0] * gain) / 2  # wh padding
...
#     coords[[1, 3]] -= pad[1]  # y padding
#     coords[:4] /= gain
#     clip_coords_v8(coords, img0_shape)
#     return coords

# def clip_coords_v8(boxes, shape):
