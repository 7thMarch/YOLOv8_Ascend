...
save_path = 'outputs/yolov5/world_cup.jpg'
...
for i in range(nums):
    ...
    T_Record = time.time()
    _img = img[:, :, ::-1].transpose(2, 0, 1)  # BGR to RGB, HWC to CHW
    _img = np.expand_dims(_img, 0).astype(np.float32)  # 将形状转换为 channel first (1, 3, 640, 640)，即扩展第一维为 batchsize
    _img = np.ascontiguousarray(_img) / 255.0  # 转换为内存连续存储的数组
    times['preProcess'] += time.time() - T_Record

    T_Record = time.time()
    _img = Tensor(_img)  # 将numpy转为转为Tensor类
    ...
    output = np.array(output)  # 将数据转为 numpy array 类型  (1,25200,85)
    ...
    T_Record = time.time()
    pred_all = boxout[0].numpy()  # 转换为numpy数组 # (3,6)
